...
class EHRAuditLogSFTTrainer:
    ...
    def evaluate_and_extract_all(self, save_dir=None):
        ...
        with (torch.no_grad()):
            ...
            for batch in tqdm(dataloader, desc="Unified Evaluation & Feature Extraction"):
                # if 'error_label' in batch:
                #     all_labels_binary.extend(batch['error_label'].tolist())
                # else:
                #     raise ValueError(
                #         "Missing 'error_label' in batch — please include it in test_data during preprocessing.")
                # if self.config['timedelta_cutoff'] is not None:
                #     if 'time_delta' in batch:
                #         time_deltas = batch['time_delta'].to(self.device)
                #     else:
                #         raise ValueError(
                #             "Missing 'time_delta' in batch — please include it in test_data during preprocessing.")
                ...
                # with torch.cuda.amp.autocast(enabled=False):
                    ...
                    # Accuracy masking (ignoring -100/padding and <ROW> tokens, and where time_deltas_flat < 1)
                    # if (self.config['timedelta_cutoff'] is not None) and self.config.get("use_prompt") and (self.config.get("instruct_text") is None):
                    #     time_deltas_flat = time_deltas.squeeze(0)
                    ...
                    #            & (time_deltas_flat >= float(self.config['timedelta_cutoff']))  # excludes positions where time_deltas < 1sec
                    # # Accuracy masking (ignoring -100/padding and <ROW> tokens)
                    # else:
                    ...
                    if (self.config.get("custom_tokenization") == False and self.config.get("use_delimiter")):
                        ...
                        # Revert back the masking logic to continue pipeline
                        # Accuracy masking (ignoring -100/padding and <ROW> tokens, and where time_deltas_flat < 1)
                        # if (self.config['timedelta_cutoff'] is not None) and self.config.get("use_prompt", False) and (self.config.get("instruct_text") is None):
                        #     mask = (labels_flat != -100) & (labels_flat != self.tokenizer.pad_token_id) & (
                        #             labels_flat != self.tokenizer.convert_tokens_to_ids("<ROW>")) \
                        #            & (labels_flat != self.tokenizer.convert_tokens_to_ids("<FIRST_ROW>")) \
                        #            & (labels_flat != self.tokenizer.convert_tokens_to_ids(
                        #         ['[TD_0]', '[TD_10]', '[TD_60]', '[TD_>60]'])) \
                        #            & (time_deltas_flat >= 1)  # excludes positions where time_deltas < 1sec
                        # # Accuracy masking (ignoring -100/padding and <ROW> tokens)
                        # else:
                        mask = (labels_flat != -100) & (labels_flat != self.tokenizer.pad_token_id) & (
                                    labels_flat != self.tokenizer.convert_tokens_to_ids("<ROW>")) \
    ...
    def debug_evaluate_and_extract_all(self):
        """
        Debugging Script for the original evaluate_and_extract_all() method
        """
        print(f"[DEBUG] Prompting active: {self.config.get('use_prompt', False)}")
        self.model.eval()
        all_correct_count, all_labels, all_ce, all_ce_values, all_ppls, all_embeddings = [], [], [], [], [], []
        ...
        #
        # all_correct_chunks, all_total_chunks = [], []
        # all_chunk_ce, all_chunk_entropy, all_chunk_topk_acc = [], [], []
        #
        # def split_chunks_by_row(batch_tensor, row_token_id):
        #     """Splits each sequence in the batch into chunks by <ROW>."""
        #     chunks = []
        ...
        with (torch.no_grad()):
            for batch in tqdm(dataloader, desc="Unified Evaluation & Feature Extraction"):
                ...
                # with torch.cuda.amp.autocast(enabled=False):
                    ...
                    # Skip first N tokens (warm-up window) for fair metric calculation
                    start_idx = self.config.get("skip_N_tokens", 0)
                    logits = logits[:, start_idx:, :]
                    labels = labels[:, start_idx:]
                    # if self.config['timedelta_cutoff'] is not None:
                    #     time_deltas = time_deltas[:, start_idx:]
                    ...
                    #         chunk_len = len(chunk)
                    #
                    #         ce_chunk = token_ce[mask][cursor:cursor + chunk_len]
                    #         ce_chunks.append(ce_chunk.mean().item())
                    #
                    #         entropy_chunk = entropy[mask][cursor:cursor + chunk_len]
                    #         entropy_chunks.append(entropy_chunk.mean().item())
                    #
                    #         topk_chunk = topk_correct[mask][cursor:cursor + chunk_len]
                    #         topk_chunk_accs.append(topk_chunk.float().mean().item())
                    #
                    #         cursor += chunk_len
                    #
                    #     all_chunk_ce.extend(ce_chunks)
                    ...
                    # # ece = self.compute_ECE(logits, labels, mask) # incorrect since batch-averaging
                    # # all_ece.append(ece)
                    # confidences = F.softmax(logits, dim=-1).max(dim=-1).values.squeeze(0)[mask]
                    # correct = (preds[mask] == labels_flat[mask]).float()
        ...
        # ce_std = np.std(all_ce_values)
        #
        # avg_ppl = np.exp(avg_ce) # exponential of the average cross entropy; perplexity is fundamentally a seq-level metric that captures overall uncertainty. cannot compute SD
        #
        # # avg_topk_acc = np.mean(all_topk_correct) #incorrectly averaging batches
        # # avg_topk_acc = sum(all_topk_correct_count) / sum(all_labels)
        # avg_topk_acc = np.mean(all_topk_corrects)
        ...
        #     avg_chunk_topk_acc = np.mean(all_chunk_topk_acc)
        #
        #     print(f"[RESULT] Chunk-Level Cross-Entropy: {avg_chunk_ce:.4f} (Perplexity: {avg_chunk_ppl:.4f})")
        #     print(f"[RESULT] Chunk-Level Entropy: {avg_chunk_entropy:.4f}")
        #     print(f"[RESULT] Chunk-Level Top-5 Accuracy: {avg_chunk_topk_acc:.4f}")
        #
        #     if self.config["log_wandb"]:
        #         wandb.log({
        #             "test/chunk_accuracy": chunk_acc,
        ...
        #             np.save(os.path.join(save_dir, "seq_error_labels-prompt.npy"), np.array(all_labels_binary))
        #             np.save(os.path.join(save_dir, "cross_entropy-prompt.npy"), np.array(all_ce, dtype=object))
        #             results_csv_path = os.path.join(save_dir, "inference_results-prompt.csv")
        #             result_col_name = self.config['HF_repo_name']+'-prompt'
        #             # Save each token-level metric across all tokens in the test set in deterministic order
        #             torch.save(torch.tensor(all_corrects), os.path.join(save_dir, "llm_token_correct.pt"))
        #         else:
        #             torch.save(torch.cat(all_embeddings, dim=0), os.path.join(save_dir, "seq_embeddings-instruction_prompt.pt"))
        #             np.save(os.path.join(save_dir, "seq_error_labels-instruction_prompt.npy"), np.array(all_labels_binary))
        #             np.save(os.path.join(save_dir, "cross_entropy-instruction_prompt.npy"), np.array(all_ce, dtype=object))
        ...
        #         # np.save(os.path.join(save_dir, "seq_error_labels.npy"), np.array(all_labels_binary))
        #         # np.save(os.path.join(save_dir, "cross_entropy.npy"), np.array(all_ce, dtype=object))
        #         results_csv_path = os.path.join(save_dir, "inference_results.csv")
        #         result_col_name = self.config['HF_repo_name']
        #         # Save each token-level metric across all tokens in the test set in deterministic order
        #         torch.save(torch.tensor(all_corrects), os.path.join(save_dir, "llm_tokens_correct.pt"))
        #         torch.save(torch.tensor(all_topk_corrects), os.path.join(save_dir, "llm_tokens_top5_correct.pt"))
        #         torch.save(torch.tensor(all_ce_values), os.path.join(save_dir, "llm_tokens_cross_entropy.pt"))
        #         torch.save(torch.tensor(all_entropy), os.path.join(save_dir, "llm_tokens_entropy.pt"))
        #         torch.save(torch.tensor(all_confidences), os.path.join(save_dir, "llm_tokens_confidence.pt"))
        ...
        #             "Field-level Entropy (chunked)": avg_chunk_entropy,
        #             "Test Accuracy": top1_acc,
        #             "Top-5 Accuracy": avg_topk_acc,
        #             "Cross-Entropy": avg_ce,
        #             "Perplexity": avg_ppl,
        #             "Entropy": avg_entropy,
        #             "ECE": global_ece
        #         }, name=result_col_name)
        ...
        #
        # return top1_acc, avg_ppl, all_ce, all_embeddings, avg_topk_acc, avg_entropy
    # def compute_ECE(self, logits, labels, mask, n_bins=10):
    #     probs = F.softmax(logits, dim=-1)  # (1, seq_len-1, vocab_size)
    #     confidences, predictions = torch.max(probs, dim=-1)  # (1, seq_len-1)
    #     correct = (predictions == labels).squeeze(0)  # (seq_len-1,)
    #
    #     confidences = confidences.squeeze(0)[mask]
